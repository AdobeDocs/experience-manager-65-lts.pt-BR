...
>[!IMPORTANT]
>
> **Hotfix Obrigatório** - Para evitar problemas de SNFE (SegmentNotFoundException) com compactação offline ao instalar o Service Pack 1, instale o hotfix descrito em [Problemas conhecidos - Corrupção do repositório durante a compactação online](#repository-corruption-during-online-compaction-after-offline-compaction-granite-65146).
...
Para obter mais informações, consulte **Migração de Javax para o Namespace de Jacarta**.
...
**Ambiente**
Aplicável a: clientes do AEM 6.5 LTS (no local) que instalam o Service Pack 1 (SP1). O SP1 é entregue como um arquivo JAR de inicialização rápida.
...
* **FORMS-24688:** Ao configurar o JBoss EAP 8.0 (AEM Forms 6.5.1 LTS) no Linux, os scripts criados ou editados em um Windows podem conter terminações de linha CRLF, causando /bin/sh^M: bad interpreter ou $&#39;\r&#39;: o comando não encontrou erros. Certifique-se de que os arquivos afetados usem terminações de linha LF. Isso pode ser feito usando ferramentas como dos2unix ou qualquer método equivalente.

* **FORMS-24689:** No modo Turnkey, a atualização da porta do banco de dados não é aplicada corretamente durante a instalação ou atualização. No modo de instalação recente, os usuários só podem selecionar entre as portas exibidas na tela. No modo Upgrade, a nova porta do banco de dados deve ser atualizada manualmente em lc_turnkey.xml antes de continuar com o upgrade.

* **FORMS-24687:** em implantações de cluster do AEM Forms 6.5 LTS no JBoss EAP 8, os arquivos domain/configuration/domain_oracle.xml, domain_mysql.xml e domain_mssql.xml podem conter uma marca `<security>` duplicada, resultando em XML inválido que impede a inicialização do Controlador de Domínio.
...
Os usuários sofreram corrupção do repositório durante a compactação online se a compactação offline fosse executada anteriormente no repositório JCR. Um `SegmentNotFoundException` (SNFE) pode ocorrer neste cenário. Isso pode causar a corrupção do repositório.

Para resolver o problema, instale o hotfix pelo Gerenciador de Pacotes da [Distribuição de Software](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.1-hotfix-GRANITE-65385-1.0.zip). Como o hotfix inclui um pacote de baixo nível `oak-segment-tar`, a instância é reiniciada após a instalação.

Planeje o tempo de inatividade de uma instância ao aplicá-la. Para compactação offline, use o [jar de execução de Oak](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar) correspondente, também disponível na Distribuição de software.

>[!NOTE]
>
> Para qualquer operação executada pela Oak, use o [Oak-run 1.78.1-B006 jar](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar) (vinculado acima).

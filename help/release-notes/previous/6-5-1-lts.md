...
>[!IMPORTANT]
>
> **Hotfix obrigatória** - Para evitar problemas de SNFE (SegmentNotFoundException) com compactação offline ao instalar o Pacote de serviços 1, instale a hotfix descrita em [Problemas conhecidos - Corrupção do repositório durante a compactação online](#repository-corruption-during-online-compaction-after-offline-compaction-granite-65146).
...
O AEM 6.5 Forms LTS para JEE agora está disponível. Para obter detalhes sobre ambientes compatíveis, consulte o documento Combinações de [plataformas compatíveis](/help/forms/using/aem-forms-jee-supported-platforms.md). Os links do instalador estão disponíveis na página [Versões do AEM Forms](https://experienceleague.adobe.com/en/docs/experience-manager-release-information/aem-release-updates/forms-updates/aem-forms-releases).

#### O que está incluído no Pacote de serviços 1 do AEM Forms 6.5 LTS

**Atualizações de suporte do Java**

Foi introduzido suporte para versões mais recentes do Java:

* Oracle Java™ 17/21 (64 bits) (Veja mais detalhes na seção [Suporte ao Java](#java-support).

**Atualizações de suporte do servidor de aplicativos**

* Foi adicionado suporte para JBoss® EAP 8.
* A estrutura de segurança PicketBox legada foi removida.
...
Para obter detalhes de configuração, consulte o guia de instalação e configuração.
...
* Jakarta EE 9 apresenta uma alteração de namespace de `javax.*` para `jakarta.*`
* Todos os **DSCs personalizados** devem ser migrados para o namespace `jakarta.*`
* O AEM Forms 6.5 LTS SP1 oferece suporte **somente a servidores de aplicativos baseados em Jakarta EE 9+**

Para obter mais informações, consulte **Migração do namespace Javax para Jakarta**.

#### Migração do namespace `javax` para `jakarta`

A partir do **AEM Forms 6.5 LTS SP1**, somente os servidores de aplicativos que implementam a **API do servlet Jakarta 5/6** têm suporte. Com o **Jakarta EE 9 e posterior**, todas as APIs passaram do namespace `javax.{}` para `jakarta.`.

Como resultado, **todos os DSCs personalizados devem usar o namespace `jakarta`**. Os componentes personalizados criados com as APIs do `javax.{}` são **incompatíveis** com os servidores de aplicativos compatíveis.

**Opções de migração para DSCs personalizados**

Você pode migrar os DSCs personalizados existentes usando uma das seguintes abordagens:

**Opção 1: Migração de código-fonte (recomendada)**

* Atualizar todas as instruções de importação de `javax.{}` para `jakarta.`
* Recriar e recompilar os projetos DSC personalizados
* Reimplantar os componentes atualizados no servidor de aplicativos
...
**Opção 2: Migração binária usando a Eclipse Transformer**

* Usar a ferramenta **Eclipse Transformer** para converter binários compilados (`.jar`, `.war`) de `javax` em `jakarta`
...
A tabela a seguir mostra as alterações comuns de namespace necessárias durante a migração de `javax` para `jakarta`:
...
Use esses mapeamentos como referência ao atualizar o código-fonte DSC personalizado ou ao validar binários transformados.
...
* Definição da estratégia de manutenção e sustentação do AEM 6.5. Essa correção incluiu o seguinte:
...
>[!NOTE]
>
> (Somente Forms) Oracle Java™ 8/11 (64 bits) necessário para instalação única.
...
**Ambiente**
Aplica-se a: clientes do AEM 6.5 LTS (No local) que instalam o Pacote de serviços 1 (SP1). O SP1 é entregue como um arquivo JAR de inicialização rápida.

**Por que isso é importante**
O SP1 para AEM 6.5 LTS é fornecido como um Quickstart JAR em vez de um arquivo ZIP para instalar usando o Gerenciador de pacotes. Clientes locais realizam a atualização por substituir o arquivo JAR de início rápido, fazer a extração do conteúdo e reiniciar. Esse método é consistente com o procedimento de atualização no local da Adobe.
...
* **FORMS-24688:** ao configurar o JBoss EAP 8.0 (AEM Forms 6.5.1 LTS) no Linux, scripts criados ou editados em Windows podem conter terminações de linha no formato CRLF, resultando em erros como /bin/sh^M: bad interpreter ou $&#39;\r&#39;: command not found. Verifique se os arquivos afetados usam terminações de linha no formato LF. Isso pode ser feito usando ferramentas como dos2unix ou qualquer método equivalente.

* **FORMS-24689:** no modo Turnkey, a atualização da porta do banco de dados não é aplicada corretamente durante a nova instalação ou atualização. No modo Nova instalação, os usuários só podem selecionar entre as portas exibidas na tela. No modo Atualização, a nova porta do banco de dados deve ser atualizada manualmente em lc_turnkey.xml antes de continuar com a atualização.

* **FORMS-24687:** em implantações de cluster do AEM Forms 6.5 LTS no JBoss EAP 8, os arquivos domain/configuration/domain_oracle.xml, domain_mysql.xml e domain_mssql.xml podem conter uma tag `<security>` duplicada, resultando em XML inválido que impede a inicialização do Controlador de domínio.
...
Usuários constataram corrupção do repositório durante a compactação online em casos em que a compactação offline havia sido executada anteriormente no repositório JCR. Pode ocorrer `SegmentNotFoundException` (SNFE) neste cenário. Isso pode causar corrupção do repositório.

Para resolver o problema, instale a hotfix por meio do Gerenciador de pacotes em [Distribuição de software](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.1-hotfix-GRANITE-65385-1.0.zip). Como a hotfix inclui um pacote `oak-segment-tar` de baixo nível, a instância é reiniciada após a instalação.

Planeje o tempo de inatividade de uma instância ao aplicá-la. Para compactação offline, use o arquivo [jar do Oak-run](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar) correspondente, também disponível na Distribuição de software.

>[!NOTE]
>
> Para qualquer operação com o Oak-run, use o arquivo [jar do Oak-run 1.78.1-B006](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/oak-run-1.78.1-B006.jar) (indicado no link acima).
...
### Pacote `com.adobe.granite.apicontroller` ausente no AEM 6.5 LTS SP1 (GRANITE-67639) {#missing-apicontroller-bundle-granite-67639}

O pacote `com.adobe.granite.apicontroller` está ausente no AEM 6.5 LTS SP1. Esse pacote controla como os pacotes OSGi são resolvidos e pode impedir que os pacotes sejam resolvidos para outros pacotes, o que é útil para limitar as APIs expostas.

Instale o hotfix de [Distribuição de Software](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html?package=/content/software-distribution/en/details.html/content/dam/aem/public/adobe/packages/cq660/hotfixes/cq-6.5.lts.1-hotfix-GRANITE-67639-1.0.zip) para usar esta funcionalidade.
...
* [Lista de pacotes da OSGi inclusos no Experience Manager 6.5 LTS, pacote de serviços 1](/help/release-notes/assets/65lts_sp1_bundles.zip) <!-- UPDATE FOR EACH NEW RELEASE -->
* [Lista de pacotes de conteúdo inclusos no Experience Manager 6.5 LTS, pacote de serviços 1](/help/release-notes/assets/65lts_sp1_packages.zip) <!-- UPDATE FOR EACH NEW RELEASE -->

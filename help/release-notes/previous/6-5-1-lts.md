...
* **FORMS-24688:** ao configurar o JBoss EAP 8.0 no Linux, scripts de shell modificados no Windows podem conter terminações de linha CRLF, causando `/bin/sh^M: bad interpreter` ou $`\r`: erro de comando não encontrado. Converta os arquivos afetados em terminações de linha LF usando dos2unix para resolver isso.

* **FORMS-24689:** No Modo de Atualização Turnkey, a atualização da porta do banco de dados em `lc_turnkey.xml` pode não ser aplicada corretamente durante a atualização, fazendo com que o processo faça referência ao valor da porta antiga.

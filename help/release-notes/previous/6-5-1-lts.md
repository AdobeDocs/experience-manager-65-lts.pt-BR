...
* **FORMS-24688:** Ao configurar o JBoss EAP 8.0 (AEM Forms 6.5.1 LTS) no Linux, os scripts modificados no Windows podem conter terminações de linha CRLF, causando /bin/sh^M: bad interpreter ou $&#39;\r&#39;: o comando não encontrou erros. Converta os arquivos .sh afetados no formato Unix (LF) usando dos2unix (por exemplo, localizar . -type f -name &quot;*.sh&quot; -exec dos2unix {} +) para resolver o problema.

* **FORMS-24689:** No Modo de Atualização Turnkey, as atualizações na porta do banco de dados não são selecionadas da interface do usuário e devem ser modificadas manualmente no arquivo de configuração lc_turnkey.xml. Se não for atualizado corretamente, o sistema pode continuar usando o valor de porta antigo.

* **FORMS-24687:** em implantações de cluster LTS do AEM Forms 6.5.1 no JBoss EAP 8, `domain/configuration/domain_oracle.xml` pode conter uma marca `<security>` duplicada, resultando em XML inválido que impede o controlador de domínio de iniciar.

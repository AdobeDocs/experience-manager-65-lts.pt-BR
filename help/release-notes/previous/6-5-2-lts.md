...
* Adição de um prompt do modo expresso do Configuration Manager (LCM) para as credenciais de administrador do AEM durante a configuração, substituindo as credenciais padrão integradas. Esse aprimoramento está disponível por meio da correção 6.5 LTS SP2 JEE. (FORMS-26704)
...
**Os seguintes problemas foram corrigidos no hotfix do AEM Forms 6.5 LTS Service Pack 2 JEE (build 6.6.0-0006). Para aplicá-los, [baixe e instale o hotfix](/help/forms/using/adobe-experience-manager-forms-65-lts-hotfixes.md).**

* Depois que o Apache Shiro foi atualizado para a versão 2.1.0, o AEM Forms no JEE falhou ao inicializar com um `NoClassDefFoundError` para o gerenciador de segurança Shiro. A inicialização agora é concluída com êxito. (FORMS-26818)
* O AEM Forms no JEE falhou com um erro &quot;nenhuma classe encontrada&quot; para `org.owasp.esapi.reference.JavaLogFactory`. A classe ausente agora é resolvida. (FORMS-26819)
* Depois de atualizar para o AEM Forms 6.5 LTS, os pontos de extremidade do TaskManager foram removidos. Os endpoints agora são retidos. (FORMS-26584, FORMS-26589)
* No JEE, a etapa MergeEars do Configuration Manager falhou com um erro de declaração DOCTYPE (`ALC-LCM-010-200`) devido ao construtor de XML seguro. MergeEars agora é concluído. (FORMS-26569)
* Os registros no nível do aplicativo estavam ausentes nas implantações do IBM® WebSphere® Liberty. O registro em log no nível do aplicativo agora está restaurado. (FORMS-25063)
* No JBoss®, o email falhou com &quot;IMAPProvider não é um subtipo&quot;. O email agora funciona no JBoss®. (FORMS-24892)
* No WebSphere® Liberty Profile (WLP), o email falhava com &quot;Não era possível converter o soquete em TLS&quot;. O email sobre TLS agora funciona no WLP. (FORMS-24692)
* Atualização da biblioteca Gibson para a versão 6.0.29665850. (FORMS-26688)
* Melhorias na validação de asserção SAML com suporte. (FORMS-25222)
* Atualização do Apache Log4j para a versão 2.25.5. (FORMS-26733, FORMS-26734)
* Essa correção também inclui correções de segurança.

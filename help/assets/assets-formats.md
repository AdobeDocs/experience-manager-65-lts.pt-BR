...
A biblioteca de transcodificação de imagens do Adobe é uma solução de processamento de imagens que executa as funções principais de tratamento de imagens, como codificação, transcodificação, reamostragem e redimensionamento.
...
| PDF | application/pdf | `pdfprocess=Rasterize&resolution=150`<br>`&colorspace=Auto&pdfbrochure=false`<br>`&keywords=false&links=false` | [OpçõesPDF](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/image-production-api/data-types/r-pdf-options.html) |
...
Consulte também o artigo da Base de conhecimento [Detectar formatos de arquivo não compatíveis com o Dynamic Media](https://helpx.adobe.com/experience-manager/kb/detect-unsupported-assets-for-dynamic-media.html).

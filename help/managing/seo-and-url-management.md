...
A SEO (Otimização do mecanismo de pesquisa) se tornou uma preocupação principal para muitos profissionais de marketing. Como resultado, as preocupações com a SEO devem ser abordadas em muitos projetos do AEM.

Este documento descreve primeiramente algumas [práticas recomendadas da SEO](#seo-best-practices) e recomendações sobre uma implementação do AEM. Em seguida, ele detalha algumas [etapas de implementação mais complexas](#aem-configurations) abordadas na primeira seção.
...
Existem algumas práticas recomendadas aceitas para URLs.

No projeto do AEM, ao avaliar os URLs, pergunte-se o seguinte:

&quot;Se um usuário visse esse URL e nenhum conteúdo na página, ele poderia descrever essa página?&quot;
...
Esta seção descreve as etapas de implementação para configurar o AEM com as seguintes recomendações de SEO.
...
A tendência nos últimos anos tem sido remover parâmetros para tornar os URLs mais legíveis. Em muitas plataformas, esse processo de remoção envolve a implementação de redirecionamentos no servidor da Web ou na Rede de entrega de conteúdo (CDN), mas o Sling torna o processo simples. Os seletores Sling:

* Melhoram a legibilidade do URL.
* Permitem o armazenamento das páginas em cache na Dispatcher e melhoram a segurança.
* Permite abordar o conteúdo diretamente, em vez de utilizar um servlet genérico para recuperar o conteúdo. Ela concede os benefícios das ACLs que você aplica ao repositório e aos filtros que você aplica no Dispatcher.
...
Os servlets **Bin** seguem o padrão para o qual muitos desenvolvedores são usados na programação J2EE. O servlet está registrado em um caminho específico que, no AEM, geralmente está em `/bin`, e você extrai os parâmetros de solicitação necessários da cadeia de caracteres de consulta.
...
* A presença de parâmetros de consulta no URL significa que o Dispatcher não pode armazenar a resposta em cache.
* Se quiser proteger esse servlet, implemente sua própria lógica de segurança personalizada no servlet.
...
Nesse caso, o recurso que a URL endereça - uma instância do recurso `myPageType` - é acessível automaticamente no servlet. Para acessá-lo, você chama o seguinte:
...
* O Dispatcher já está configurado para distribuir esse conteúdo por meio de uma função de distribuição de site. Nenhuma configuração adicional é necessária.

### Regravação de URL {#url-rewriting}

No AEM, todas as páginas da Web são armazenadas no `/content/my-brand/my-content`. Embora esse local seja útil da perspectiva do gerenciamento de dados de repositório, não é necessariamente a forma como você deseja que os clientes vejam o site. Além disso, pode entrar em conflito com a orientação da SEO de manter os URLs o mais curtos possível. Além disso, você pode estar distribuindo vários sites da mesma instância do AEM e de nomes de domínio diferentes.
...
>[!NOTE]
>
>A propriedade `sling:alias` pode ser definida usando a propriedade [Alias ao editar as Propriedades da página](/help/sites-authoring/editing-page-properties.md#advanced).
...
Embora essa abordagem esteja documentada na documentação oficial do AEM e Sling, o suporte regular à expressão fornecido por essa implementação é limitado no escopo quando comparado às opções que estão disponíveis usando o `SlingResourceResolver` diretamente. Além disso, a implementação de mapeamentos desta maneira pode levar a problemas com a invalidação do cache do Dispatcher.
...
No entanto, há também uma maneira mais simples de gerenciar esse problema:
...
1. **Regras SlingResourceResolver**
   ...
   * para apenas `/my-page.html`
   ...
   Essa conversão está de acordo com a prática recomendada de manter os URLs o mais curtos possível.

1. **Mapeamento da saída de URL nas páginas**

   Depois de definir os mapeamentos no Apache Sling Resource Resolver, use esses mapeamentos em seus componentes para garantir que os URLs enviados nas páginas sejam curtos e organizados. Você pode fazer essa manutenção usando a função de mapa do `ResourceResolver`.
...
Até o momento, você implementou mapeamentos juntamente com a lógica nos componentes para usar esses mapeamentos ao enviar URLs para páginas.

A peça final do quebra-cabeça é lidar com estes URLs mais curtos quando eles chegam ao Dispatcher, que é onde o `mod_rewrite` entra em cena. O maior benefício de usar o `mod_rewrite` é que os URLs são mapeados de volta na sua forma longa *antes* de serem enviados ao módulo do Dispatcher. Esse fluxo significa que o Dispatcher solicita o URL longo do servidor de publicação e o armazena em cache de acordo. Portanto, todas as solicitações de liberação do Dispatcher que chegarem do servidor de publicação poderão invalidar esse conteúdo com êxito.
...
Os mecanismos de pesquisa *devem* verificar a presença de um arquivo `robots.txt` na raiz do site antes de rastrear o site. Embora os principais mecanismos de pesquisa, como Google, Yahoo ou Bing, respeitem esse arquivo, alguns mecanismos de pesquisa estrangeiros não respeitam.
...
O trabalho de geração de mapa de site pode ser executado em instâncias de nível do autor e de publicação. Normalmente, é recomendável executar a geração em instâncias do nível de publicação, já que URLs canônicos adequados podem ser gerados somente lá (devido às regras de Mapeamento de recursos do Sling que geralmente estão presentes apenas em instâncias do nível de publicação). No entanto, é possível conectar uma implementação personalizada do mecanismo de externalização usado para gerar as URLs canônicas implementando a interface [SitemapLinkExternalizer](https://javadoc.io/doc/com.adobe.cq.wcm/com.adobe.aem.wcm.seo/latest/com/adobe/aem/wcm/seo/sitemap/externalizer/SitemapLinkExternalizer.html). Se uma implementação personalizada puder gerar as URLs canônicas de um mapa de site nas instâncias do nível do autor, o `SitemapScheduler` poderá ser configurado para o modo de execução do autor. E a carga de trabalho de geração do mapa de site XML pode ser distribuída entre as instâncias do cluster de serviços do autor. Nesse cenário, deve-se ter cuidado ao manipular conteúdo que ainda não foi publicado, foi modificado ou está visível apenas para um grupo restrito de usuários.

O AEM Sites contém uma implementação padrão de um `SitemapGenerator` que atravessa uma árvore de páginas para gerar um mapa de site. Ela é pré-configurada para produzir apenas os URLs canônicos de um site e qualquer alternativa de idioma que estiver disponível. Ela também pode ser configurada para incluir a data da última modificação de uma página, se necessário. Para fazer isso, habilite a opção _Adicionar modificada por último_ da configuração _Adobe AEM SEO - Gerador de mapas do site da árvore de páginas_ e selecione uma _Última Source modificada_. Quando os mapas do site são gerados no nível de publicação, é recomendável usar a data `cq:lastModified`.

...
description: Saiba como migrar conteúdo do AEM 6.5 para o AEM 6.5 LTS usando a ferramenta de atualização do Oak
feature: Upgrading
...
1. Ferramenta Oak-upgrade: baixe o jar `oak-upgrade` do [repositório Maven oficial](https://mvnrepository.com/artifact/org.apache.jackrabbit/oak-upgrade). Verifique se a versão corresponde à versão principal do Oak usada no AEM 6.5 LTS. A ferramenta de atualização da Oak é executada no Oracle® Java™ 11 ou posterior

## Processo de migração {#step-by-step-migration-process}
...
### Uso da ferramenta de atualização do Oak para migração de conteúdo {#using-the-oak-upgrade-tool-for-content-migration}
...
Ao migrar um repositório SegmentMK antigo (anterior ao Oak 1.6) para um novo SegmentMK (versão do Oak superior ou igual a 1.6), os pontos de verificação também são migrados. Esse processo evita a reindexação quando o Oak está sendo executado pela primeira vez no novo repositório. No entanto, os pontos de verificação não são migrados nos seguintes casos:

1. Os caminhos de inclusão, exclusão ou mesclagem personalizados são especificados ou
1. O sistema copia os binários por referência. Nenhum armazenamento de dados de origem é especificado e dois pontos de verificação diferentes contêm um binário diferente no mesmo caminho.
...
O aviso também pode ser ignorado, mas nesse caso, o repositório é totalmente reindexado na primeira inicialização. Pode ser um processo longo, especialmente para a instância grande. O repositório não poderá ser usado até que o processo de reindexação seja concluído. Use a opção `--skip-checkpoints` para suprimir o aviso.
...
Para obter mais informações sobre a ferramenta de atualização do Oak e o uso avançado, consulte a [documentação oficial](https://jackrabbit.apache.org/oak/docs/migration.html).

...
description: Saiba como definir configurações de tipo de arquivo. No PDF Generator, é possível definir a configuração do aplicativo para tipos de arquivo compatíveis para definir as configurações de tipo de arquivo.
...
No PDF Generator, é possível definir as configurações do aplicativo para tipos de arquivos compatíveis. No Windows, você pode definir as configurações do aplicativo para cada tipo de arquivo compatível. No UNIX e Linux, é possível definir as configurações do aplicativo HTML para PDF e OpenOffice.
...
* Especifique quais configurações de tipo de arquivo usar por padrão (consulte [Importação e exportação de arquivos de configuração do PDF Generator](/help/forms/using/admin-help/importing-exporting-pdf-generator-configuration.md))
...
>[!NOTE]
>
>As configurações de tipo de arquivo não estão disponíveis para os conversores de fallback, como Acrobat para HTML para conversão de PDF, Microsoft PowerPoint, Microsoft Word e Microsoft Excel.

## Criar ou editar configurações de Tipo de Arquivo {#create-or-edit-file-type-settings}

Crie ou edite uma configuração de tipo de arquivo para especificar como o aplicativo lida com a conversão de tipos de arquivo compatíveis. No Windows, você pode definir as configurações do aplicativo para cada tipo de arquivo compatível. No UNIX e Linux, é possível definir as configurações do aplicativo HTML para PDF e OpenOffice.
...
**Reduzir Resolução de Imagens:** Diminui o número de pixels em imagens coloridas, em tons de cinza e monocromáticas. A redução da resolução de imagens digitalizadas é executada após a conclusão do OCR. O padrão é Menor (600 dpi). Essa opção não estará disponível se você definir o estilo de saída do PDF como Imagem pesquisável (Exata).
...
1. Marque a caixa de seleção ao lado da configuração a ser excluída. Você pode selecionar várias origens. As configurações que não têm uma caixa de seleção ao lado delas são sempre incluídas com o PDF Generator e não podem ser excluídas.
1. Clique em **[!UICONTROL Excluir]** e, na página Confirmação de Exclusão, clique em **[!UICONTROL Excluir]**.

## Imagem para configurações do PDF {#image-to-pdf-settings}

As opções a seguir determinam como os arquivos de imagem são convertidos para o PDF. Para obter instruções sobre como acessar essas configurações, consulte [Criar ou editar configurações de tipo de arquivo](configuring-file-type-settings.md#create-or-edit-file-type-settings).

**Extensões de Nome de Arquivo:** Lista separada por vírgulas de extensões de nome de arquivo que podem ser convertidas.

**Tentar o Conversor de Fallback:** o PDF Generator pode usar o Java™ ou o Acrobat para converter arquivos de imagem para o PDF. Quando essa opção é selecionada e uma conversão falha ou atinge o limite de tempo especificado, o PDF Generator tenta a conversão usando o método alternativo. Se o método alternativo falhar ou atingir o tempo limite especificado, uma exceção será gravada no arquivo de log.

>[!NOTE]
>
>Os arquivos do JPEG 2000 só podem ser convertidos usando o Acrobat.
...
**Reduzir Resolução de Imagens:** Diminui o número de pixels em imagens coloridas, em escala de cinza e monocromáticas após a conclusão do OCR. Escolha o grau de redução de resolução a ser aplicado. As opções com números mais altos reduzem a resolução, o que produz PDFs de resolução mais alta.

## Configurações de exportação do Adobe PDF (somente Windows) {#adobe-pdf-export-settings-windows-only}

A configuração Tipo de arquivo de exportação na seção de configurações de exportação do Adobe PDF é usada para converter um arquivo do PDF em outro formato. O padrão é o HTML 4.01 com folhas de estilos em cascata (CSS) 1.0 (*.htm, *.html).
...
As opções a seguir determinam como os arquivos do HTML são convertidos para o PDF. Para obter instruções sobre como acessar essas opções, consulte [Criar ou editar configurações de tipo de arquivo](configuring-file-type-settings.md#create-or-edit-file-type-settings).
...
*Aranha* verifica nas páginas da Web os links para outras páginas da Web. Quando um link para outra página da Web é encontrado, a página de destino é buscada e incluída no documento do PDF que é gerado. Ative essas opções para definir o número de níveis que serão buscados e convertidos no PDF:
...
Ative essas opções para especificar como as páginas do HTML são convertidas. Com base no tamanho da página, os valores de largura, altura e margem são ajustados de acordo.
...
**Adicionar marcadores ao PDF:** Adiciona marcadores ao documento do PDF.
...
Ative essas opções para especificar como exibir conteúdo, como exibir páginas no documento do PDF e como especificar o nível de ampliação:
...
**Ampliação:** escolha a ampliação predefinida para a exibição inicial do documento do PDF ou selecione um valor personalizado. A escolha de uma configuração padrão indica que a ampliação padrão do Acrobat é usada.

**Abrir em Número de Página:** Especifique o número de página para o qual o PDF abre.
...
**Ocultar barra de menus:** oculta a barra de menus no documento do PDF.

**Ocultar barras de ferramentas:** oculta as barras de ferramentas no documento do PDF.

**Ocultar Controles de Janela:** Oculta os controles de janela no documento do PDF.

## Fazer vídeos Flash nas configurações do PDF {#flash-videos-to-pdf-settings}

O PDF Generator suporta a capacidade de enviar um vídeo para o Adobe Flash (arquivo SWF ou FLV) e criar um arquivo PDF com um vídeo para o Adobe Flash incorporado. Essa conversão não requer que o Adobe Flash Player esteja instalado no Forms Server. Para obter instruções sobre como acessar esta opção, consulte [Criar ou editar configurações de tipo de arquivo](configuring-file-type-settings.md#create-or-edit-file-type-settings).

**Extensões de Nome de Arquivo:** Lista separada por vírgulas de extensões de nome de arquivo que podem ser convertidas.

## Configurações de XPS para PDF {#xps-to-pdf-settings}

XML Paper Specification (XPS) é usado na máquina de impressão do Windows. Este é um formato do Microsoft e pode ser criado a partir de qualquer aplicativo do Microsoft Office. O AEM Forms permite converter arquivos XPS em PDF.
...
## configurações do otimizador do PDF {#pdf-optimizer-settings}

O PDF Generator oferece suporte à capacidade de reduzir o tamanho dos arquivos PDF. Se você usa todas essas configurações ou apenas algumas dependem de como você pretende usar os arquivos e das propriedades essenciais que um arquivo deve ter. Na maioria dos casos, as configurações padrão são apropriadas para eficiência máxima - economizando espaço removendo fontes incorporadas, compactando imagens e removendo itens dos arquivos que não são mais necessários.
...
Se o documento do PDF incluir um trabalho artístico que contenha transparência, você poderá usar as configurações do PDF Otimizer para nivelar a transparência e reduzir o tamanho do arquivo.

>[!NOTE]
>
>Se o Acrobat 4.0 e posterior for selecionado como a versão do Target PDF, todos os objetos transparentes serão nivelados. Para outras versões do Target PDF, a transparência é compatível e você pode definir as configurações de transparência.

Selecione **Transparência** para definir as configurações de transparência ao otimizar documentos do PDF.
...
* Selecione **Descartar Objetos** para especificar objetos a serem removidos da PDF e para otimizar linhas curvas em desenhos CAD.
...
* **Descartar Todas as Ações do JavaScript**: remove da PDF as ações que usam o JavaScript.
...
* **Converter Linhas Suaves em Curvas**: reduz o número de pontos de controle usados para criar curvas em desenhos CAD, o que resulta em arquivos PDF menores e em uma renderização de tela mais rápida.
...
* **Descartar Todas as Imagens Alternativas**: Remove todas as versões de uma imagem, exceto a versão destinada à exibição na tela. Alguns PDFs incluem várias versões da mesma imagem para diferentes propósitos, como visualização em tela de baixa resolução e impressão de alta resolução.
...
* **Descartar Todos os Comentários, Forms e Multimídia**: remove todos os comentários, formulários, campos de formulário e multimídia da PDF.
* **Descartar Todos os Dados do Objeto**: remove todos os objetos do PDF.
* **Descartar Referências Cruzadas Externas**: remove os links para outros documentos. Os links que saltam para outros locais na PDF não são removidos.
...
* **Descartar anexos de arquivo**: remove todos os anexos de arquivo, incluindo anexos adicionados à PDF como comentários. (O PDF Otimizer não otimiza arquivos anexados.)
* **Descartar Dados Privados de Outros Aplicativos**: elimina informações de um documento do PDF que sejam úteis somente para o aplicativo que criou o documento. Essa configuração não afeta a funcionalidade do PDF, mas diminui o tamanho do arquivo.
...
**Descartar Destinos Nomeados Não Referenciados**: Remove destinos nomeados que não estão sendo referenciados internamente de dentro do documento do PDF. Essa opção não verifica links de outros arquivos ou sites da PDF.

**Otimizar o PDF para o Modo de Exibição Rápido da Web**: reestrutura um documento do PDF para download de página por vez (serviço de bytes) de servidores Web.
...
**Tentar OpenOffice como Conversor de Fallback**: quando esta opção estiver selecionada e uma conversão usando o Microsoft Excel falhar ou atingir o tempo limite especificado, o PDF Generator tentará a conversão usando OpenOffice. Se a conversão usando OpenOffice falhar ou atingir o limite de tempo especificado, uma exceção será gravada no arquivo de log.
...
**Criar arquivo compatível com PDF/A-1a**: força o uso da configuração PDF/A-1b:2005 do RGB Adobe PDF.
...
**Converter Informações do Documento**: adiciona propriedades de documento do PDF com base nas informações do documento no arquivo de origem. Isso inclui informações como título do documento, autor, assunto e palavras-chave.

**Adicionar Links ao Adobe PDF**: converte hiperlinks no arquivo de origem em hiperlinks no documento do PDF.

**Anexar arquivo do Source ao Adobe PDF**: quando esta opção é selecionada, a planilha original do Excel é inserida como um anexo dentro do documento do PDF gerado.

**Habilitar Acessibilidade e Refluxo com Adobe PDF Marcado**: incorpora marcas no documento PDF para habilitar a acessibilidade e o refluxo.
...
**[!UICONTROL Tentar OpenOffice como Conversor de Fallback]**: quando esta opção é selecionada e uma conversão usando o Microsoft PowerPoint falha ou atinge o limite de tempo limite especificado, o PDF Generator tenta a conversão usando OpenOffice. Se a conversão usando OpenOffice falhar ou atingir o limite de tempo especificado, uma exceção será gravada no arquivo de log.
...
**[!UICONTROL Anexar arquivo do Source ao Adobe PDF]**: adiciona o arquivo de origem ao arquivo do PDF como um anexo. Essa opção é desmarcada por padrão.

**[!UICONTROL Habilitar Acessibilidade E Refluxo Com Adobe PDF Marcado]**: Incorpora marcas no arquivo PDF. Essa opção é desmarcada por padrão.

**[!UICONTROL Converter multimídia em multimídia do PDF]**: converte multimídia em multimídia do PDF, quando possível. Essa opção é selecionada por padrão.
...
**[!UICONTROL Layout do PDF com base nas Configurações de Impressora do PowerPoint]**: usa as configurações de impressora do PowerPoint para criar o layout do documento do PDF.
...
1. **[!UICONTROL Anexar arquivo do Source ao Adobe PDF]**: adiciona o arquivo de origem ao arquivo do PDF como um anexo.
1. **[!UICONTROL Criar arquivo compatível com PDF/A-1a]**: força o uso da configuração PDF/A-1b:2005 do RGB Adobe PDF.
...
**[!UICONTROL Tentar OpenOffice como Conversor de Fallback]**: quando esta opção é selecionada e uma conversão usando o Microsoft Word falha ou atinge o limite de tempo especificado, o PDF Generator tenta a conversão usando OpenOffice. Se a conversão usando OpenOffice falhar ou atingir o limite de tempo especificado, uma exceção será gravada no arquivo de log.
...
**[!UICONTROL Anexar arquivo do Source ao Adobe PDF]**: adiciona o arquivo de origem ao arquivo do PDF como um anexo.
...
**[!UICONTROL Preservar Marcação do Documento no Adobe PDF]**: converte a marcação no documento do Word em anotações no arquivo do PDF.

**[!UICONTROL Adicionar Links ao Adobe PDF]**: converte hiperlinks no arquivo de origem em hiperlinks no documento do PDF.

**[!UICONTROL Converter Links de Notas de Rodapé e Notas de Fim]**: cria links a partir de citações de notas de rodapé e notas de fim para notas no documento do PDF.
...
**[!UICONTROL Converter todos os estilos em marcadores]**: converte todos os estilos do documento do Word em marcadores no documento do PDF.

**[!UICONTROL Converter estilos especificados em marcadores]**: converte os estilos definidos no campo **[!UICONTROL Estilos com níveis]** em marcadores no documento do PDF.

**[!UICONTROL Estilos com Níveis]**: especifica quais estilos no documento do Word são convertidos em marcadores no documento do PDF. Especifica também o nível dos marcadores. Para usar este recurso, desmarque a opção **[!UICONTROL Converter todos os estilos em marcadores]** e especifique os nomes dos estilos no seguinte formato:
...
**Anexar arquivo do Source ao Adobe PDF**: adiciona o arquivo de origem ao arquivo do PDF como um anexo.
...
**Abrir painel de camadas quando visualizado no Adobe Acrobat**: se as camadas do Visio não estiverem niveladas, abre uma janela onde você pode especificar as camadas que são preservadas no arquivo do PDF quando abertas usando o Acrobat. Essa opção é selecionada por padrão.

**Criar arquivo compatível com PDF/A-1b**: força o uso da Configuração do Adobe PDF PDF/A-1b:2005 (RGB).

**Converter comentários em comentários do Adobe PDF**: converte as anotações do Visio em comentários do PDF.
...
**[!UICONTROL Converter todos os layouts]**: inclui todos os layouts na PDF.
...
**[!UICONTROL Criar arquivo compatível com PDF/A-1b]**: força o uso da configuração do Adobe PDF PDF/A-1b.

**[!UICONTROL Converter todas as camadas]**: por padrão, o PDF Generator converte somente a camada padrão de arquivos do AutoCAD para o PDF, em vez de todas as camadas do arquivo. Selecione essa opção para converter todas as camadas do arquivo.

**[!UICONTROL Incorporar Informações de Escala]**: Preserva as informações de escala de desenho.

**[!UICONTROL Converter layout atual]**: inclui somente o layout atual na PDF.
...
**PDF com marcas de formatação**: permite a criação de PDF com marcas de formatação a partir de documentos OpenOffice. O PDF marcado contém informações sobre a estrutura do conteúdo do documento. Isso pode ajudar ao exibir o documento em dispositivos com telas diferentes e ao usar um software de leitor de tela. Também ajuda o software de acessibilidade a executar várias operações úteis com o documento do PDF, como ler em voz alta o conteúdo do documento do PDF.

**Exportar Anotações**: Converte as anotações em documentos OpenOffice para anotações no documento PDF gerado.

**Usar Efeitos de Transição**: converte os efeitos de transição de slides em apresentações do OpenOffice em efeitos de transição do PDF correspondentes.

**Enviar Forms no Formato**: cria um formulário do PDF que pode ser preenchido e impresso pelo usuário do documento do PDF.

**Exportar Páginas em Branco Inseridas Automaticamente**: quando esta opção é selecionada, as páginas em branco inseridas automaticamente são incluídas no documento PDF gerado. Isso é útil se você estiver imprimindo um documento do PDF com dois lados. Por exemplo, um livro pode ser configurado de modo que a primeira página do capítulo sempre comece em uma página ímpar. Se o capítulo anterior terminar em uma página ímpar, o OpenOffice inserirá uma página em branco com número par. Essa opção controla se essa página par deve ser incluída no PDF gerado.

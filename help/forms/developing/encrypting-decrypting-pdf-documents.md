...
1. Definir opções de tempo de execução de criptografia.
   ...
   * Especifique a opção de compatibilidade do Acrobat atribuindo um valor de enumeração `PasswordEncryptionCompatibility` ao membro de dados `compatibility` do objeto `PasswordEncryptionOptionSpec`. Por exemplo, atribua `PasswordEncryptionCompatibility.ACRO_7` a esse membro de dados.
